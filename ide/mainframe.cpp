...
//	ON_COMMAND( ID_AUTODOC,helpAutodoc )
...
#ifdef BETA
	if( CMenu *menu=GetMenu()->GetSubMenu( 4 ) ){
		menu->AppendMenu( MF_ENABLED,ID_AUTODOC,"Command Autodoc" );
	}
#endif
...
#endif
#ifdef PLUS
	SetWindowText( ("BlitzPlus - "+s ).c_str() );
	return;
#endif
...
Html_Help *MainFrame::getHelp( int n ){
	map<CWnd*,Html_Help*>::iterator it=helps.find( tabber.getTabWnd( n ) );
	return it==helps.end() ? 0 : it->second;
}

Html_Help *MainFrame::getHelp(){
	return getHelp( tabber.getCurrent() );
}

Html_Help *MainFrame::findHelp(){
	int n;
	Html_Help *h;
...
void MainFrame::helpOpen( Html_Help *help,const string &file ){
	open( file );
}

void MainFrame::helpTitleChange( Html_Help *help,const string &title ){
	if( Html_Help *h=getHelp() ) setTitle( h->getTitle() );
}
...
void MainFrame::helpBack(){
	if( Html_Help *h=findHelp() ) h->GoBack();
}

void MainFrame::helpForward(){
	if( Html_Help *h=findHelp() ) h->GoForward();
}
