...
Const stretchy=True;False

winw=512:winh=480
Global window=CreateWindow( "My Window!",ClientWidth(Desktop())/2-winw/2,ClientHeight(Desktop())/2-winh/2,winw,winh,Desktop(),32+7 )

menu=WindowMenu(window)

game=CreateMenu( "Game",0,menu )
Global game_new=CreateMenu( "New game",1,game )
Global game_end=CreateMenu( "End game",3,game ):DisableMenu game_end
CreateMenu( "",0,game )
CreateMenu( "Quit",4,game )

help=CreateMenu( "Help",0,menu )
about=CreateMenu( "About",101,help )
	
UpdateWindowMenu window

Global canvas=CreateCanvas( 0,0,ClientWidth(window),ClientHeight(window),window )
If stretchy SetGadgetLayout canvas,1,1,1,1
SetBuffer CanvasBuffer(canvas)

ActivateGadget canvas

Global width=GadgetWidth(canvas),height=GadgetHeight(canvas),num_rots=64
...
Global mini_ship,insectoids_image,tmp_image
...
Type Player
	Field x,y,state,lives,bang,score
End Type
...
Function UpdateFormation()
	...
	t#=Sin( c_phase )*.5+.5
	c_xs=t*c_xsize+2:c_ys=t*c_ysize+2
...
Function RenderGame()
	...
	Case 0
		...
		If game_timer<150 Or (game_timer-150) Mod 80<40
			Color 255,255,255
			TitleText( "USE GAME MENU TO START",width/2,height-FontHeight()*2,game_timer )
		EndIf
	Case 1
...
Function LoadGraphics()
	...
	player_image=LoadImage( "graphics\player.bmp" ):ScaleImage player_image,1,1
	mini_ship=CopyImage( player_image ):ScaleImage mini_ship,.4,.4
	bull_image=LoadImage( "graphics\bullet.bmp" ):ScaleImage bull_image,.5,1
	bomb_image=LoadImage( "graphics\bbomb.bmp" ):ScaleImage bomb_image,.6,.6
	boom_anim=LoadAnimImage( "graphics\kaboom.bmp",60,48,0,6 )
	insectoids_image=LoadImage( "graphics\insectoids_logo.bmp" )
	ResizeImage insectoids_image,400,128
	ResizeImage boom_anim,8,8:ResizeImage boom_anim,32,32
	
	i=LoadImage( "graphics\alien.bmp" )
	ScaleImage i,.6,.6
...
Function ExplodeText( x,y,txt$,xn#,yn# )
	...
	For ty=0 To h-1 Step 3
		For tx=0 To w-1 Step 3
			xo#=(tx-w/2)*xn
			yo#=(ty-h/2)*yn
			DrawImageRect tmp_image,x+xo,y+yo,tx,ty,3,3
		Next
	Next
